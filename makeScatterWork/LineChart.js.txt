...
var LineChart = /** @class */ (function (_super) {
    ...
    function LineChart() {
        ...
        _this.renderDots = function (_a) {
            ...
            data.forEach(function (dataset) {
                ...
                labels.forEach((label) => {
                    labelsNumbers.push(Number(label));
                });

                const maxXValue = Math.max(...labelsNumbers);
                const minXValue = Math.min(...labelsNumbers);
                const leftBound = paddingRight;
                const rightBound = paddingRight + ((dataset.data.length - 1) * (width - paddingRight)) / xMax;
                const rangeOfPos = rightBound - leftBound;
                ...
                dataset.data.forEach(function (x, i) {
                    ...
                    var percentage = (labelsNumbers[i] - minXValue) / (maxXValue - minXValue);
                    var posX = rangeOfPos * percentage + leftBound;
                    cx = posX;
        ...
        _this.renderLine = function (_a) {
            ...
            const minXValue = Math.min(...labelsNumbers);
            const leftBound = paddingRight;
            const rightBound = ((labelsNumbers.length - 1) * (width - paddingRight)) / xMax + paddingRight;
            const rangeOfPos = rightBound - leftBound;
            ...
            data.forEach(function (dataset, index) {
                var points = dataset.data.map(function (d, i) {
                    if (d === null)
                        return lastPoint;
                    // var x = (labelsAsNumber[i]-paddingRight) / xMax + paddingRight;//(i * (width - paddingRight)) / xMax + paddingRight;
    ...
    LineChart.prototype.render = function () {
        ...
        <Svg height={height + paddingBottom + legendOffset} width={width - margin * 2 - marginRight}>
          <Rect width="100%" height={height + paddingBottom + legendOffset} rx={borderRadius} ry={borderRadius} fill="url(#backgroundGradient)" fillOpacity={transparent ? 0 : 1}/>
          ...
          <G x="0" y={legendOffset}>
            ...
            <G>
              {this.renderLine(__assign(__assign(__assign({}, config), chartConfig), { paddingRight: paddingRight, paddingTop: paddingTop, data: data.datasets, indexes: data.indexes, labels: data.labels }))}
            </G>
            <G>
              {withShadow &&
            ...
            <G>
              {withDots &&
            this.renderDots(__assign(__assign({}, config), { data: data.datasets, paddingTop: paddingTop, paddingRight: paddingRight, onDataPointClick: onDataPointClick, labels: data.labels }))}
            </G>
            <G>
              {withScrollableDot &&
